...
for iFile in iFiles:
   ...
   #-------------------------------------------------------------------------------
    ...
    u_rF    = gauss2d(u_r,       lambdaTh, lambdaZ, r, th, z)
    u_thF   = gauss2d(u_th,      lambdaTh, lambdaZ, r, th, z)
    u_zF    = gauss2d(u_z,       lambdaTh, lambdaZ, r, th, z)
    u_rRF   = gauss2d(u_r*u_r,   lambdaTh, lambdaZ, r, th, z)
    u_rThF  = gauss2d(u_r*u_th,  lambdaTh, lambdaZ, r, th, z)
    u_rZF   = gauss2d(u_r*u_z,   lambdaTh, lambdaZ, r, th, z)
    u_thThF = gauss2d(u_th*u_th, lambdaTh, lambdaZ, r, th, z)
    u_thZF  = gauss2d(u_th*u_z,  lambdaTh, lambdaZ, r, th, z)
    u_zZF   = gauss2d(u_z*u_z,   lambdaTh, lambdaZ, r, th, z)
    ...
    # Back scatter and Forward scatter events
    eB = np.zeros((u_z.shape))
    eF = np.zeros((u_z.shape))
    for k in range(nz-1):
      for j in range(nth-1):
        for i in range(nr-1):
         if pi[i,j,k]<0: #back scatter
          eB[i,j,k] = pi[i,j,k]
         else:
          eB[i,j,k] = 0.0
          #-------------------------------------------------------------------------------
         if pi[i,j,k]>0: #forward scatter
          eF[i,j,k] = pi[i,j,k]
         else:
          eF[i,j,k] = 0.0
    #-------------------------------------------------------------------------------
    print('-------------------------------------------------------------------------')
    #-------------------------------------------------------------------------------
    fnam = 'efluxGauss_pipe0002_'+'{:08d}'.format(iFile)+'.h5'
    ...
    eFlux.create_dataset("pi", data=np.array(pi).transpose(0,2,1))

    eFluxB=out.create_group("fields/eB")
    eFluxB.create_dataset("eB", data=np.array(eB).transpose(0,2,1))

    eFluxF=out.create_group("fields/eF")
    eFluxF.create_dataset("eF", data=np.array(eF).transpose(0,2,1))


    UVS=out.create_group("fields/uvS")
    UVS.create_dataset("uvS", data=np.array(uvS).transpose(0,2,1))

    UVE=out.create_group("fields/uvE")
    UVE.create_dataset("uvE", data=np.array(uvE).transpose(0,2,1))
   ...
   # """Return a pretty-printed XML string for the Element."""
    ...
    attributePiB=ET.SubElement(grid, "Attribute")
    attributePiB.set("Name", "eB")
    attributePiB.set("AttributeType", "Scalar")
    attributePiB.set("Center","Node")

    dataItemPiB=ET.SubElement(attributePiB, "DataItem")
    dataItemPiB.set("Dimensions",str(len(r))+" "+str(len(z))+" "+str(len(th)))
    dataItemPiB.set("NumberType", "Float")
    dataItemPiB.set("Precision", "8")
    dataItemPiB.set("Format", "HDF")
    dataItemPiB.text = fnam+":/fields/eB/eB"

    attributePiF=ET.SubElement(grid, "Attribute")
    attributePiF.set("Name", "eF")
    attributePiF.set("AttributeType", "Scalar")
    attributePiF.set("Center","Node")

    dataItemPiF=ET.SubElement(attributePiF, "DataItem")
    dataItemPiF.set("Dimensions",str(len(r))+" "+str(len(z))+" "+str(len(th)))
    dataItemPiF.set("NumberType", "Float")
    dataItemPiF.set("Precision", "8")
    dataItemPiF.set("Format", "HDF")
    dataItemPiF.text = fnam+":/fields/eF/eF"

    attributeUVS=ET.SubElement(grid, "Attribute")
    attributeUVS.set("Name", "uvS")
    attributeUVS.set("AttributeType", "Scalar")
    attributeUVS.set("Center","Node")

    dataItemUVS=ET.SubElement(attributeUVS, "DataItem")
    dataItemUVS.set("Dimensions",str(len(r))+" "+str(len(z))+" "+str(len(th)))
    dataItemUVS.set("NumberType", "Float")
    dataItemUVS.set("Precision", "8")
    dataItemUVS.set("Format", "HDF")
    dataItemUVS.text = fnam+":/fields/uvS/uvS"

    attributeUVE=ET.SubElement(grid, "Attribute")
    attributeUVE.set("Name", "uvE")
    attributeUVE.set("AttributeType", "Scalar")
    attributeUVE.set("Center","Node")

    dataItemUVE=ET.SubElement(attributeUVE, "DataItem")
    dataItemUVE.set("Dimensions",str(len(r))+" "+str(len(z))+" "+str(len(th)))
    dataItemUVE.set("NumberType", "Float")
    dataItemUVE.set("Precision", "8")
    dataItemUVE.set("Format", "HDF")
    dataItemUVE.text = fnam+":/fields/uvE/uvE"

  
    fnam = 'efluxGauss&UV_pipe0002_'+'{:08d}'.format(iFile)+'.xmf'
