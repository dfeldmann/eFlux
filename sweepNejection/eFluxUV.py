...
nt = 0
acy5   = np.zeros(nz) # initialized for the correlation coefficient
acy15  = np.zeros(nz) # initialized for the correlation coefficient
acy50  = np.zeros(nz) # initialized for the correlation coefficient
acy100 = np.zeros(nz) # initialized for the correlation coefficient
...
#    np.seterr(divide='ignore', invalid='ignore')
    ...
    for l in range(nth):
        ...
        uv5   = uv[k, l, :]
        acy5  = acy5  + crosscorr(uv5,   eF) # compute auto-correaltion and sum up average      
    ...
    for l in range(nth):
        eF    = pi[k, l, :]  # extract 2d filtered eflux 
        uv15  = uv[k, l, :]
        acy15 = acy15  + crosscorr(uv15, eF) # compute auto-correaltion and sum up average      
    ...
    for l in range(nth):
        eF    = pi[k, l, :]  # extract 2d filtered eflux 
        uv50  = uv[k, l, :]
        acy50 = acy50  + crosscorr(uv50,  eF) # compute auto-correaltion and sum up average      
    ...
    for l in range(nth):
        eF     = pi[k, l, :]  # extract 2d filtered eflux 
        uv100  = uv[k, l, :]
        acy100 = acy100  + crosscorr(uv100, eF) # compute auto-correaltion and sum up average      
...
fnam = 'crosscoeFluxFourierUV1d'+'{:08d}'.format(iFirst)+'to'+'{:08d}'.format(iLast)+'nt'+'{:04d}'.format(nt)+'.dat'
print('-------------------------------------------------------------------------')
...
ax1.set_ylabel(r"$C_{(u'_z u'_r) \pi}$")
ax1.set_ylim([-0.1, 1.0])
ax1.plot(dz, acy100, color=Blue, linestyle='-') # .format(lambdaTh, lambdaZ))
#ax1.legend(loc='best')
ax1.title.set_text(r"$y^+ = 100$")

# plot one-dimensional two-point auto-correlation
ax2 = plt.subplot2grid((4, 1), (1, 0), rowspan=1, colspan=1)
ax2.set_ylabel(r"$C_{(u'_z u'_r) \pi}$")
ax2.set_ylim([-0.1, 1.0])
ax2.plot(dz, acy50, color=Blue, linestyle='-') # .format(lambdaTh, lambdaZ))
#ax2.legend(loc='best')
ax2.title.set_text(r"$y^+ = 50$")

# plot one-dimensional two-point auto-correlation
ax3 = plt.subplot2grid((4, 1), (2, 0), rowspan=1, colspan=1)
ax3.set_ylabel(r"$C_{(u'_z u'_r) \pi}$")
ax3.plot(dz, acy15, color=Blue, linestyle='-') # .format(lambdaTh, lambdaZ))
#ax3.legend(loc='best')
ax3.title.set_text(r"$y^+ = 15$")

# plot one-dimensional two-point auto-correlation
ax4 = plt.subplot2grid((4, 1), (3, 0), rowspan=1, colspan=1)
ax4.set_xlabel(r"$\Delta z$ in $R$")
ax4.set_ylabel(r"$C_{(u'_z u'_r) \pi}$")
ax4.plot(dz, acy5, color=Blue, linestyle='-') # .format(lambdaTh, lambdaZ))
#ax4.legend(loc='best')
ax4.title.set_text(r"$y^+ = 5$")
...
if plot != 2:
 ...
 fig.tight_layout()
 fnam = 'crosscoeFluxFourierUV1d'+'{:08d}'.format(iFirst)+'to'+'{:08d}'.format(iLast)+'nt'+'{:04d}'.format(nt)+'.pdf'
 plt.savefig(fnam)
