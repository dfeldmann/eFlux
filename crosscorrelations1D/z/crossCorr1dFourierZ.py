...
#           velocity (both filtered and unfiltered) and fourier filtered eflux, axial
#           vorticity and the Fourier filtered eflux in axial direction. 
...
#           of snapshots 
# ----------------------------------------------------------------------------------
# Usage:    python crossCorr1dFourierZ.py 
...
nt = 0
acUzPi   = np.zeros(nz) # initialized for the correlation coefficient
acUzFPi  = np.zeros(nz) # initialized for the correlation coefficient
acOmgZPi = np.zeros(nz) # initialized for the correlation coefficient
acUzOmgZ = np.zeros(nz)
...
for iFile in iFiles:
    ...
    for l in range(nth):
        uz     = u_z  [k, l, :]  # extract 2d axial signal
...
acUzPi   = acUzPi/(nth*nt)
acUzFPi  = acUzFPi/(nth*nt)
acOmgZPi = acOmgZPi/(nth*nt)
acUzOmgZ = acUzOmgZ/(nth*nt)
print('-------------------------------------------------------------------------')
...
fnam = 'crossCorr1dFourierZ'+'{:08d}'.format(iFirst)+'to'+'{:08d}'.format(iLast)+'nt'+'{:04d}'.format(nt)+'.dat'
print('-------------------------------------------------------------------------')
print('Writing cross-correlation to file', fnam)
f = open(fnam, 'w')
f.write('# One-dimensional two-point cross-correlations in axial (z) direction\n')
f.write('# For Uz, Uz(Fourier Filtered), eFlux and axial vorticity component.\n')
...
for i in range(nz-1):
 f.write("%23.16e %23.16e %23.16e %23.16e %23.16e\n" % (dz[i], acUzPi[i], acUzFPi[i], acOmgZPi[i], acUzOmgZ[i]))
f.close()
