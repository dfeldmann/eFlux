#!/usr/bin/env python3
#====================================================================================
# Purpose:  Computes the cross-correlation coefficient between the azimuthal fluctuating 
#           velocity (both filtered and unfiltered) and fourier filtered eflux, azimuthal
#           vorticity and the Fourier filtered eflux in azimuthal direction. 
...
#           of snapshots 
# ----------------------------------------------------------------------------------
# Usage:    python crossCorr1dFourierTh.py 
...
lambdaZp  =  75   # streamwise azimuthal direction (z)
...
nt = 0
acUzPi   = np.zeros(nth) # initialized for the correlation coefficient
acUzFPi  = np.zeros(nth) # initialized for the correlation coefficient
acOmgZPi = np.zeros(nth) # initialized for the correlation coefficient
acUzOmgZ = np.zeros(nth)
...
for iFile in iFiles:
    ...
    u_rF    = f.fourier2d(u_r,       lambdaTh, lambdaZ, r, th, z, 1)
    u_thF   = f.fourier2d(u_th,      lambdaTh, lambdaZ, r, th, z, 1)
    u_zF    = f.fourier2d(u_z,       lambdaTh, lambdaZ, r, th, z, 1)
    u_rRF   = f.fourier2d(u_r*u_r,   lambdaTh, lambdaZ, r, th, z, 1)
    u_rThF  = f.fourier2d(u_r*u_th,  lambdaTh, lambdaZ, r, th, z, 1)
    u_rZF   = f.fourier2d(u_r*u_z,   lambdaTh, lambdaZ, r, th, z, 1)
    u_thThF = f.fourier2d(u_th*u_th, lambdaTh, lambdaZ, r, th, z, 1)
    u_thZF  = f.fourier2d(u_th*u_z,  lambdaTh, lambdaZ, r, th, z, 1)
    u_zZF   = f.fourier2d(u_z*u_z,   lambdaTh, lambdaZ, r, th, z, 1)
    ...
    for l in range(nz):
        uz     = u_z  [k, :, l]  # extract 2d azimuthal signal
        eF     = pi   [k, :, l]  # extract 2d filtered eflux 
        uzF    = u_zF [k, :, l]
        omg    = omgZ [k, :, l]

        acUzPi    = acUzPi   + crosscorr(uz,  eF)  
        acUzFPi   = acUzFPi  + crosscorr(uzF, eF) 
        acOmgZPi  = acOmgZPi + crosscorr(omg, eF)
        acUzOmgZ  = acUzOmgZ + crosscorr(uz, omg)
...
acUzPi   = acUzPi/(nz*nt)
acUzFPi  = acUzFPi/(nz*nt)
acOmgZPi = acOmgZPi/(nz*nt)
acUzOmgZ = acUzOmgZ/(nz*nt)
print('-------------------------------------------------------------------------')
...
fnam = 'crossCorr1dFourierTh'+'{:08d}'.format(iFirst)+'to'+'{:08d}'.format(iLast)+'nt'+'{:04d}'.format(nt)+'.dat'
print('-------------------------------------------------------------------------')
print('Writing cross-correlation to file', fnam)
f = open(fnam, 'w')
f.write('# One-dimensional two-point cross-correlations in azimuthal (theta) direction\n')
f.write('# For Uz, Uz(Fourier Filtered), eFlux and azimuthal vorticity component.\n')
...
f.write("# 01st column: azimuthal separation dz in units of pipe radii (R)\n")
f.write("# 02nd column: UzPi correlation in units of RMS\n")
f.write("# 03rd column: UzFPi correlation in units of RMS\n")
f.write("# 04th column: OmgZPi correlation in units of RMS\n")
f.write("# 03rd column: UzOmgZ correlation in units of RMS\n")
for i in range(nth-1):
 f.write("%23.16e %23.16e %23.16e %23.16e %23.16e\n" % (dth[i], acUzPi[i], acUzFPi[i], acOmgZPi[i], acUzOmgZ[i]))
f.close()
...
#-----------------------------------------------------------------------------------
# plot first azimuthal velocity signal
...
# plot second azimuthal velocity signal
...
if plot != 2:
 ...
 fig.tight_layout()
 fnam = 'piCorr1dFourierZ'+'{:08d}'.format(iFirst)+'to'+'{:08d}'.format(iLast)+'nt'+'{:04d}'.format(nt)+'.pdf'
 plt.savefig(fnam)
