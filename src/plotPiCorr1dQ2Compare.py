...
#           ejection (Q2) events. Plot azimtuhal (theta) and axial (z)
...
# Usage:    python plotPiCorr1dQ2Compare.py
...
print('Plot 1d cross-correlations between energy flux Q2 events based on different kernels.')
...
# read axial correlation with ejection (Q2) events for Fourier filtered eFlux from ascii file
...
# read axial correlation with ejection (Q2) events for Gauss filtered eFlux from ascii file
...
# read axial correlation with ejection (Q2) events for Box filtered eFlux from ascii file
...
# read azimuthal correlation with ejection (Q2) events for Fourier filtered eFlux from ascii file
...
# read azimuthal correlation with ejection (Q2) events for Gauss filtered eFlux from ascii file
...
# read azimuthal correlation with ejection (Q2) events for Box filtered eFlux from ascii file
...
ax1.set_ylabel(r"$C_{\Pi Q_2}$")
ax1.set_ylim(bottom=-0.10, top=0.12)
...
ax2.set_ylim(bottom=-0.10, top=0.12)
...
if plot == 1:
 ...
 fnam = str.replace(fnam, 'piCorrThQ4Q2Box2d', 'plotPiCorr1dQ2Compare')
 plt.savefig(fnam)
