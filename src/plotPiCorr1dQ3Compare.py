...
#           inward (Q3) interactions. Plot azimtuhal (theta) and axial (z)
...
# Usage:    python plotPiCorr1dQ3Compare.py
...
print('Plot 1d cross-correlations between energy flux and Q3 events based on different kernels.')
...
cthF = np.loadtxt(fnam)[:, 8] # 9th column: Cross-correlation Q3 with Pi
...
cthG = np.loadtxt(fnam)[:, 8] # 9th column: Cross-correlation Q3 with Pi
...
cthB = np.loadtxt(fnam)[:, 8] # 9th column: Cross-correlation Q3 with Pi
...
czF = np.loadtxt(fnam)[:, 8] # 9th column: Cross-correlation Q3 with Pi
...
czG = np.loadtxt(fnam)[:, 8] # 9th column: Cross-correlation Q3 with Pi
...
czB = np.loadtxt(fnam)[:, 8] # 9th column: Cross-correlation Q3 with Pi
...
ax1.set_ylabel(r"$C_{\Pi Q_3}$")
ax1.set_ylim(bottom=-0.18, top=0.18)
...
ax2.set_ylim(bottom=-0.18, top=0.18)
...
ip = InsetPosition(ax2, [0.55, 0.05, 0.4, 0.4]) # Manually set position and relative size of the inset within ax2
...
if plot == 1:
 ...
 fnam = str.replace(fnam, 'piCorrZQsBox2d', 'plotPiCorr1dQ3Compare')
 plt.savefig(fnam)
