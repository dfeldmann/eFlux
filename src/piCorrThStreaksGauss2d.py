...
#           Additionally, separate forward (piP) from backward (PiPfluxes and high-speed
#           from low-speed streaks to compute cross-correlations  and c
...
# Usage:    python piCorrThStreaksGauss2d.py
# Authors:  Daniel Feldmann, Mohammad Umair, Jan Chen
# Date:     28th March 2019
# Modified: 11th July 2020
...
print('Compute eFlux (Gauss) and 1d azimuthal correlations with streaks for', len(iFiles), 'snapshot(s):', iFiles[0], 'to', iFiles[-1])
...
acUzF       = np.zeros(nth) # initialise auto-correlation for u'_z Gauss filtered
...
ccUzpPi     = np.zeros(nth) # initialise cross-correlation for u'_z > 0 and eFlux, high-speed only
ccUznPi     = np.zeros(nth) # initialise cross-correlation for u'_z < 0 and eFlux, low-speed only
...
for iFile in iFiles:
    ...
    for l in range(nz):
        ...
        # sum up first and second statistical moments in time and (homogeneous) theta direction for normalisation
        uz1     = uz1     + np.sum(uz1d)
...
ccUzpPi     = ccUzpPi     / (nt*nz)
ccUznPi     = ccUznPi     / (nt*nz)
...
fnam = 'piCorrThStreaksGauss2d_pipe0002_'+'{:08d}'.format(iFirst)+'to'+'{:08d}'.format(iLast)+'nt'+'{:04d}'.format(nt)+'.dat'
f = open(fnam, 'w')
