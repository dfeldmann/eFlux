...
#           plane for the negative energy flux (Pi < 0) and the axial velocity
#           component (u'_z) representing streamwise streaks. Correlation maps
#           are read from several ascii files and plotted for three different
...
# Usage:    python plotPiCorr2d1dStreaksCompareNegative.py
# Authors:  Daniel Feldmann, Mohammad Umair
# Date:     28th March 2019
# Modified: 03rd January 2020
...
print('Plot 1d and 2d cross-correlations between negative energy flux and streaks based on different kernels.')
...
puthF = np.loadtxt(fnam)[:, 11] # 12th column: Cross-correlation u'_z with Pi < 0
...
print('Reading 1d cross-correlation from', fnam)
puthG = np.loadtxt(fnam)[:, 11] # 12th column: Cross-correlation u'_z with Pi < 0
...
print('Reading 1d cross-correlation from', fnam)
puthB = np.loadtxt(fnam)[:, 11] # 12th column: Cross-correlation u'_z with Pi < 0
...
puzF = np.loadtxt(fnam)[:, 11] # 12th column: Cross-correlation u'_z with Pi < 0
...
print('Reading 1d cross-correlation from', fnam)
puzG = np.loadtxt(fnam)[:, 11] # 12th column: Cross-correlation u'_z with Pi < 0
...
print('Reading 1d cross-correlation from', fnam)
puzB = np.loadtxt(fnam)[:, 11] # 12th column: Cross-correlation u'_z with Pi < 0
...
f  = np.loadtxt(fnam)[:, 12] # 13th column: Cross-correlation u'_z with Pi < 0
...
print('Reading 2d cross-correlation from', fnam)
g  = np.loadtxt(fnam)[:, 12] # 13th column: Cross-correlation u'_z with Pi < 0
...
print('Reading 2d cross-correlation from', fnam)
b  = np.loadtxt(fnam)[:, 12] # 13th column: Cross-correlation u'_z with Pi < 0
...
ax1.set_ylabel(r"$\Delta\theta r^{+}$ in $\sfrac{\nu}{u_{\tau}}$")
...
ax2.set_ylabel(r"$\Delta\theta r^{+}$ in $\sfrac{\nu}{u_{\tau}}$")
...
ax3.set_ylabel(r"$\Delta\theta r^{+}$ in $\sfrac{\nu}{u_{\tau}}$")
...
ax4.set_xlabel(r"$C_{u^{\prime}_{z}\Pi_{<0}}$")
ax4.set_xlim(left=-0.27, right=0.09)
#ax4.set_xticks([0.00, 0.10])
...
ax4.plot(puthF, th1d, color=Black,      linestyle='-', marker='^', markevery=[192], zorder=7, label=r"Fourier")
ax4.plot(puthG, th1d, color=Vermillion, linestyle='-', marker='o', markevery=[192], zorder=9, label=r"Gauss", markersize=2.2)
ax4.plot(puthB, th1d, color=Blue,       linestyle='-', marker='s', markevery=[192], zorder=8, label=r"Box")
...
ax4.text(0.08, 0.9, r"d)", ha="left", va="top", transform=ax4.transAxes) #, rotation=0, bbox=labelBox)
...
ax5.set_ylabel(r"$C_{u^{\prime}_{z}\Pi_{<0}}$")
ax5.set_ylim(bottom=-0.30, top=0.05)
...
ax5.plot(z1d, puzG, color=Vermillion, linestyle='-', marker='o', markevery=[1134], zorder=9, label=r"Gauss", markersize=2.2)
ax5.plot(z1d, puzB, color=Blue,       linestyle='-', marker='s', markevery=[1143], zorder=8, label=r"Box")
...
ip = InsetPosition(ax5, [0.70, 0.30, 0.20, 0.60]) # Manually set position and relative size of the inset within original ax5
ax6.set_axes_locator(ip)
ax6.set_xlim(left=-10.0, right=10.0) # where to zoom in?
ax6.set_ylim(bottom=-0.2, top=0.05) # where to zoom in?
ax6.set_xticks([])
ax6.set_yticks([])
ax6.axhline(y=0.0, color=Grey)
ax6.axvline(x=0.0, color=Grey)
ax6.plot(z1d, puzF, color=Black,      linestyle='-', marker='^', markevery=[1153], zorder=7, label=r"Fourier")
ax6.plot(z1d, puzG, color=Vermillion, linestyle='-', marker='o', markevery=[1150], zorder=9, label=r"Gauss", markersize=2.2)
ax6.plot(z1d, puzB, color=Blue,       linestyle='-', marker='s', markevery=[1151], zorder=8, label=r"Box")
...
if plot != 2:
 ...
 fnam = str.replace(fnam, 'piCorrThZStreaksBox2d', 'plotPiCorr2d1dStreaksCompareNegative')
 plt.savefig(fnam)
