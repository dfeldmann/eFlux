...
# Usage:    python piCorrZStreaksFourier2d.py
...
print('Compute eFlux (Fourier) and 1d axial correlations with streaks for', len(iFiles), 'snapshot(s):', iFiles[0], 'to', iFiles[-1])
...
acUzF       = np.zeros(nz) # initialise auto-correlation for u'_z Fourier filtered
...
fnam = 'piCorrZStreaksFourier2d_pipe0002_'+'{:08d}'.format(iFirst)+'to'+'{:08d}'.format(iLast)+'nt'+'{:04d}'.format(nt)+'.dat'
f = open(fnam, 'w')
...
ax1.text(-1000.0, 0.75, "Fourier", ha="center", va="center")
