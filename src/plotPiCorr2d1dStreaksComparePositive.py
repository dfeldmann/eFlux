...
#           plane for the postive energy flux (Pi > 0) and the axial velocity
...
# Usage:    python plotPiCorr2d1dStreaksComparePositive.py
...
print('Plot 1d and 2d cross-correlations between positive energy flux and streaks based on different kernels.')
...
puthF = np.loadtxt(fnam)[:, 8] # 9th column: Cross-correlation u'_z with Pi > 0
...
print('Reading 1d cross-correlation from', fnam)
puthG = np.loadtxt(fnam)[:, 8] # 9th column: Cross-correlation u'_z with Pi > 0
...
print('Reading 1d cross-correlation from', fnam)
puthB = np.loadtxt(fnam)[:, 8] # 9th column: Cross-correlation u'_z with Pi > 0
...
puzF = np.loadtxt(fnam)[:, 8] # 9th column: Cross-correlation u'_z with Pi > 0
...
print('Reading 1d cross-correlation from', fnam)
puzG = np.loadtxt(fnam)[:, 8] # 9th column: Cross-correlation u'_z with Pi > 0
...
print('Reading 1d cross-correlation from', fnam)
puzB = np.loadtxt(fnam)[:, 8] # 9th column: Cross-correlation u'_z with Pi > 0
...
f  = np.loadtxt(fnam)[:, 9] # 10th column: Cross-correlation u'_z with Pi > 0
...
print('Reading 2d cross-correlation from', fnam)
g = np.loadtxt(fnam)[:, 9] # 10th column: Cross-correlation u'_z with Pi > 0
...
print('Reading 2d cross-correlation from', fnam)
b = np.loadtxt(fnam)[:, 9] # 10th column: Cross-correlation u'_z with Pi > 0
...
ax4.set_xlim(left=-0.09, right=0.16)
...
ax5.set_ylim(bottom=-0.20, top=0.15)
ax5.set_yticks([-0.2, -0.1, 0.0, 0.1])
...
ip = InsetPosition(ax5, [0.75, 0.35, 0.20, 0.60]) # Manually set position and relative size of the inset within original ax5
...
if plot != 2:
 ...
 fnam = str.replace(fnam, 'piCorrThZStreaksBox2d', 'plotPiCorr2d1dStreaksComparePositive')
 plt.savefig(fnam)
