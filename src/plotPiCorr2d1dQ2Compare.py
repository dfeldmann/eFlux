...
#           plane for the energy flux (Pi) and Q2 events representing typical
#           ejection events of the near-wall cycle. Correlation maps are read
...
# Usage:    python plotPiCorr2d1dQ2Compare.py
...
print('Plot 1d and 2d cross-correlations between energy flux and Q2 (ejection) events based on different kernels.')
...
# read 1d azimuthal cross-correlation with Q2 events for Fourier filtered eFlux from ascii file
...
pqthF = np.loadtxt(fnam)[:, 7] # 8th column: Cross-correlation Q2 with Pi

# read 1d azimuthal cross-correlation with Q2 events for Gauss filtered eFlux from ascii file
fnam = 'piCorrThQsGauss2d_pipe0002_00570000to01675000nt0222.dat'
print('Reading 1d cross-correlation from', fnam)
pqthG = np.loadtxt(fnam)[:, 7] # 8th column: Cross-correlation Q2 with Pi

# read 1d azimuthal cross-correlation with Q2 events for box filtered eFlux from ascii file
fnam = 'piCorrThQsBox2d_pipe0002_00570000to01675000nt0222.dat'
print('Reading 1d cross-correlation from', fnam)
pqthB = np.loadtxt(fnam)[:, 7] # 8th column: Cross-correlation Q2 with Pi

# read 1d axial cross-correlation with Q2 events for Fourier filtered eFlux from ascii file
fnam = 'piCorrZQsFourier2d_pipe0002_00570000to01675000nt0222.dat'
print('Reading 1d cross-correlation from', fnam)
z1d  = np.loadtxt(fnam)[:, 0] # 1st column: Axial separation, only once
pqzF = np.loadtxt(fnam)[:, 7] # 8th column: Cross-correlation Q2 with Pi

# read 1d axial cross-correlation with Q2 events for Gauss filtered eFlux from ascii file
fnam = 'piCorrZQsGauss2d_pipe0002_00570000to01675000nt0222.dat'
print('Reading 1d cross-correlation from', fnam)
pqzG = np.loadtxt(fnam)[:, 7] # 8th column: Cross-correlation Q2 with Pi

# read 1d axial cross-correlation with Q2 events for box filtered eFlux from ascii file
fnam = 'piCorrZQsBox2d_pipe0002_00570000to01675000nt0222.dat'
print('Reading 1d cross-correlation from', fnam)
pqzB = np.loadtxt(fnam)[:, 7] # 8th column: Cross-correlation Q2 with Pi
...
# read 2d cross-correlation with Q2 events for Fourier filtered eFlux from ascii file
...
f  = np.loadtxt(fnam)[:, 8] # 9th column: Cross-correlation for Q2 and Pi

# read 2d cross-correlation with Q2 events for Gauss filtered eFlux from ascii file
...
print('Reading 2d cross-correlations from', fnam)
g  = np.loadtxt(fnam)[:, 8] # 9th column: Cross-correlation for Q2 and Pi

# read 2d cross-correlation with Q2 events for box filtered eFlux from ascii file
fnam = 'piCorrThZQsBox2d_pipe0002_01675000to01675000nt0001.dat'
fnam = 'piCorrThZQsBox2d_pipe0002_00570000to01675000nt0222.dat'
print('Reading 2d cross-correlations from', fnam)
b  = np.loadtxt(fnam)[:, 8] # 9th column: Cross-correlation for Q2 and Pi
...
ax4.set_xlabel(r"$C_{Q_{2}\Pi}$")
ax4.set_xlim(left=-0.15, right=0.15)
ax4.set_xticks([-0.10, 0.0, 0.1])
...
ax4.text(0.03, 0.9, r"d)", ha="left", va="top", transform=ax4.transAxes) #, rotation=0, bbox=labelBox)
...
ax5.set_xticks([-800, -400, 0, 400, 800])
ax5.minorticks_on()
ax5.set_ylabel(r"$C_{Q_{2}\Pi}$")
...
cb1.ax.set_ylabel(r"$C_{Q_{2}\Pi}$")
...
if plot != 2:
 ...
 fnam = str.replace(fnam, 'piCorrThZQsBox2d', 'plotPiCorr2d1dQ2Compare')
 plt.savefig(fnam)
