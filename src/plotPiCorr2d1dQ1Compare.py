...
#           plane for the energy flux (Pi) and Q1 events representing typical
#           outward interactions of the near-wall cycle. Correlation maps are
...
# Usage:    python plotPiCorr2d1dQ1Compare.py
...
print('Plot 1d and 2d cross-correlations between energy flux and Q1 (outward) interactions based on different kernels.')
...
pqthF = np.loadtxt(fnam)[:, 6] # 7th column: Cross-correlation Q1 with Pi

# read 1d azimuthal cross-correlation with Q1 events for Gauss filtered eFlux from ascii file
fnam = 'piCorrThQsGauss2d_pipe0002_00570000to01675000nt0222.dat'
print('Reading 1d cross-correlation from', fnam)
pqthG = np.loadtxt(fnam)[:, 6] # 7th column: Cross-correlation Q1 with Pi

# read 1d azimuthal cross-correlation with Q1 events for box filtered eFlux from ascii file
fnam = 'piCorrThQsBox2d_pipe0002_00570000to01675000nt0222.dat'
print('Reading 1d cross-correlation from', fnam)
pqthB = np.loadtxt(fnam)[:, 6] # 7th column: Cross-correlation Q1 with Pi

# read 1d axial cross-correlation with Q1 events for Fourier filtered eFlux from ascii file
fnam = 'piCorrZQsFourier2d_pipe0002_00570000to01675000nt0222.dat'
print('Reading 1d cross-correlation from', fnam)
z1d  = np.loadtxt(fnam)[:, 0] # 1st column: Axial separation, only once
pqzF = np.loadtxt(fnam)[:, 6] # 7th column: Cross-correlation Q1 with Pi

# read 1d axial cross-correlation with Q1 events for Gauss filtered eFlux from ascii file
fnam = 'piCorrZQsGauss2d_pipe0002_00570000to01675000nt0222.dat'
print('Reading 1d cross-correlation from', fnam)
pqzG = np.loadtxt(fnam)[:, 6] # 7th column: Cross-correlation Q1 with Pi

# read 1d axial cross-correlation with Q1 events for box filtered eFlux from ascii file
...
# read 2d cross-correlation with Q1 events for Fourier filtered eFlux from ascii file
...
f  = np.loadtxt(fnam)[:, 7] # 8th column: Cross-correlation for Q1 and Pi

# read 2d cross-correlation with Q1 events for Gauss filtered eFlux from ascii file
...
print('Reading 2d cross-correlations from', fnam)
g  = np.loadtxt(fnam)[:, 7] # 8th column: Cross-correlation for Q1 and Pi

# read 2d cross-correlation with Q1 events for box filtered eFlux from ascii file
fnam = 'piCorrThZQsBox2d_pipe0002_01675000to01675000nt0001.dat'
fnam = 'piCorrThZQsBox2d_pipe0002_00570000to01675000nt0222.dat'
print('Reading 2d cross-correlations from', fnam)
b  = np.loadtxt(fnam)[:, 7] # 8th column: Cross-correlation for Q1 and Pi
...
ax4.set_xlabel(r"$C_{Q_{1}\Pi}$")
ax4.set_xlim(left=-0.10, right=0.05)
...
ax5.set_xticks([-500, 0, 500, 1000])
ax5.minorticks_on()
ax5.set_ylabel(r"$C_{Q_{1}\Pi}$")
ax5.set_ylim(bottom=-0.175, top=0.175)
...
cb1.ax.set_ylabel(r"$C_{Q_{1}\Pi}$")
...
if plot != 2:
 ...
 fnam = str.replace(fnam, 'piCorrThZQsBox2d', 'plotPiCorr2d1dQ1Compare')
 plt.savefig(fnam)
