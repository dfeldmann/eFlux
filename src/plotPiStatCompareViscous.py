...
# Usage:    python plotPiStatComapreViscous.py 
...
ax1.set_xticks([])
ax1.set_ylabel(r"$\langle \Pi^{\prime} \rangle_{\theta, z, t}$ in $ $")
#ax1.set_ylim(bottom=-0.010, top=0.035)
ax1.axhline(y=0.0, color=Grey)
ax1.plot(yp[:-3], eFMeanBox[:-3]*fpi,     color=Blue,        linestyle='-',  zorder=9, label=r"Box")
ax1.plot(yp[:-3], eFMeanGauss[:-3]*fpi,   color=Vermillion,  linestyle='-',  zorder=8, label=r"Gauss")
ax1.plot(yp[:-3], eFMeanFourier[:-3]*fpi, color=Black,       linestyle='-',  zorder=7, label=r"Fourier")
...
ax2.plot(yp[:-3], eFRmsBox[:-3]*fpi,     color=Blue,        linestyle='-',  label=r"Box")
ax2.plot(yp[:-3], eFRmsGauss[:-3]*fpi,   color=Vermillion,  linestyle='-',  label=r"Gauss")
ax2.plot(yp[:-3], eFRmsFourier[:-3]*fpi, color=Black,       linestyle='-',  label=r"Fourier")
...
ax3.set_ylabel(r"$\text{Skewness}\left(\Pi^{\prime}\right)$ in $\text{RMS}^{3}$")
ax3.axhline(y=0.0, color=Grey)
ax3.plot(yp[:-1], eFSkewBox[:-1],     color=Blue,        linestyle='-',  zorder=9, label=r"Box")
ax3.plot(yp[:-1], eFSkewGauss[:-1],   color=Vermillion,  linestyle='-',  zorder=8, label=r"Gauss")
ax3.plot(yp[:-1], eFSkewFourier[:-1], color=Black, linestyle='-',  zorder=7, label=r"Fourier")
...
ax4.set_ylabel(r"$\text{Flatness}\left(\Pi^{\prime}\right)$ in $\text{RMS}^{4}$")
ax4.set_ylim(bottom=0.0, top=500.0)
ax4.plot(yp[:-1], eFFlatBox[:-1],     color=Blue,        linestyle='-',  zorder=9, label=r"Box")
ax4.plot(yp[:-1], eFFlatGauss[:-1],   color=Vermillion,  linestyle='-',  zorder=8, label=r"Gauss")
ax4.plot(yp[:-1], eFFlatFourier[:-1], color=Black,       linestyle='-',  zorder=7, label=r"Fourier")
