...
# multiplying the Box transform of one function by the complex conjugate of the
# Box transform of the other gives the Box transform of their correlation.
#-----------------------------------------------------------------------------------
...
for iFile in iFiles:
    ...
    eFy5    = pi   [k, :, :]  # extract 2d filtered eflux 
    print('-------------------------------------------------------------------------')
    k = 63
    print ("Wall-normal plane at y+ =", (1-r[k])*180.4)
    ...
    k = 49
    print ("Wall-normal plane at y+ =", (1-r[k])*180.4)
    eFy50    = pi   [k, :, :]  # extract 2d filtered eflux 
    print('-------------------------------------------------------------------------')
    k = 33
    print ("Wall-normal plane at y+ =", (1-r[k])*180.4)
    print('-------------------------------------------------------------------------')
    eFy100   = pi   [k, :, :]  # extract 2d filtered eflux 
...
eFy5am   = np.max(np.abs(eFy5  ))
eFy15am  = np.max(np.abs(eFy15 ))
eFy50am  = np.max(np.abs(eFy50 ))
eFy100am = np.max(np.abs(eFy100))

omgy5am   = np.max(np.abs(omgy5  ))
omgy15am  = np.max(np.abs(omgy15 ))
omgy50am  = np.max(np.abs(omgy50 ))
omgy100am = np.max(np.abs(omgy100))
...
cl1 = np.linspace(-eFy5am, +eFy5am, ncl) # set contour levels manually
cf1 = ax1.contourf(z, th, eFy5, cl1,  cmap=comap) # , extend='both')
cl2 = np.linspace(-omgy5am, +omgy5am, 5) # set contour levels manually
#cf2 = ax1.contour(z, th, omgy5, cl2,  cmap='bwr', linestyles='dashed', linewidths=0.5)
...
cb1 = plt.colorbar(cf1, cax=cbx, ticks=[-eFy5am, 0, +eFy5am]) # set colorbar scale
cb1.set_label(r"${\displaystyle\pi}$ ")
ax1.title.set_text(r"$y^+ = 5$")

ax2 = plt.subplot2grid((4, 1), (1, 0), rowspan=1, colspan=1)
#ax2.set_xlabel(r"${\Delta z}$ in $d$")
ax2.set_ylabel(r"${r\Delta\theta}$ in $d$")
cl3 = np.linspace(-eFy15am, +eFy15am, ncl) # set contour levels manually
cf3 = ax2.contourf(z, th, eFy15, cl3,  cmap=comap) # , extend='both')
cl4 = np.linspace(-omgy15am, +omgy5am, 5) # set contour levels manually
#cf4 = ax2.contour(z, th, omgy15, cl4,  cmap='bwr', linestyles='dashed', linewidths=0.5)
ax2.set_aspect('equal') # makes axis ratio natural
dvr = make_axes_locatable(ax2) # devider
cbx = dvr.append_axes("right", size="5%", pad=0.1) # make colorbar axis
cb2 = plt.colorbar(cf3, cax=cbx, ticks=[-eFy15am, 0, +eFy15am]) # set colorbar scale
cb2.set_label(r"${\displaystyle\pi}$ ")
ax2.title.set_text(r"$y^+ = 15$")


ax3 = plt.subplot2grid((4, 1), (2, 0), rowspan=1, colspan=1)
#ax3.set_xlabel(r"${\Delta z}$ in $d$")
ax3.set_ylabel(r"${r\Delta\theta}$ in $d$")
cl5 = np.linspace(-eFy50am, +eFy50am, ncl) # set contour levels manually
cf5 = ax3.contourf(z, th, eFy50, cl5,  cmap=comap) # , extend='both')
cl6 = np.linspace(-omgy50am, +omgy50am, 5) # set contour levels manually
#cf6 = ax3.contour(z, th, omgy50, cl6,  cmap='bwr', linestyles='dashed', linewidths=0.5)
ax3.set_aspect('equal') # makes axis ratio natural
dvr = make_axes_locatable(ax3) # devider
cbx = dvr.append_axes("right", size="5%", pad=0.1) # make colorbar axis
cb3 = plt.colorbar(cf5, cax=cbx, ticks=[-eFy50am, 0, +eFy50am]) # set colorbar scale
cb3.set_label(r"${\displaystyle\pi}$ ")
ax3.title.set_text(r"$y^+ = 50$")


ax4 = plt.subplot2grid((4, 1), (3, 0), rowspan=1, colspan=1)
ax4.set_xlabel(r"${\Delta z}$ in $d$")
ax4.set_ylabel(r"${r\Delta\theta}$ in $d$")
cl7 = np.linspace(-eFy100am, +eFy100am, ncl) # set contour levels manually
cf7 = ax4.contourf(z, th, eFy100, cl7,  cmap=comap) # , extend='both')
cl8 = np.linspace(-omgy100am, +omgy100am, 5) # set contour levels manually
#cf8 = ax4.contour(z, th, omgy100, cl8,  cmap='bwr', linestyles='dashed', linewidths=0.5)
ax4.set_aspect('equal') # makes axis ratio natural
dvr = make_axes_locatable(ax4) # devider
cbx = dvr.append_axes("right", size="5%", pad=0.1) # make colorbar axis
cb4 = plt.colorbar(cf7, cax=cbx, ticks=[-eFy100am, 0, +eFy100am]) # set colorbar scale
cb4.set_label(r"${\displaystyle\pi}$ ")
ax4.title.set_text(r"$y^+ = 100$")
...
if plot != 2:
 ...
 fig.tight_layout()
 fnam = 'eflux2DBox'+'{:08d}'.format(iLast)+'.pdf'
# with PdfPages(fnam) as pp
# pp.savefig()
 plt.savefig(fnam)
