...
#           of snapshots 
# ----------------------------------------------------------------------------------
# Usage:    python crossCorr2DFourier.py 
...
nt = 0
acUzPi   = np.zeros((nth,nz)) # initialized for the correlation coefficient
acUzFPi  = np.zeros((nth,nz)) # initialized for the correlation coefficient
acOmgZPi = np.zeros((nth,nz)) # initialized for the correlation coefficient
acUzOmgZ = np.zeros((nth,nz))
...
for iFile in iFiles:
    ...
    # extract data and compute cross-correlations    
    uz     = u_z  [k, :, :]  
    eF     = pi   [k, :, :]
    uzF    = u_zF [k, :, :]
    omg    = omgZ [k, :, :]
...
fnam = 'crossCorr2dFourier'+'{:08d}'.format(iFirst)+'to'+'{:08d}'.format(iLast)+'nt'+'{:04d}'.format(nt)+'.dat'
print('-------------------------------------------------------------------------')
...
for i in range(nth-1):
 for j in range(nz-1):
  f.write("%23.16e %23.16e %23.16e %23.16e %23.16e %23.16e\n" % (rdth[i], dz[j], acUzPi[i,j], acUzFPi[i,j], acOmgZPi[i,j], acUzOmgZ[i,j]))
f.close()
print('-------------------------------------------------------------------------')
print('Written 2D cross-correlation to the file', fnam)
print('-------------------------------------------------------------------------')
#=================================================================================
...
# plot one-dimensional two-point cross-correlation
