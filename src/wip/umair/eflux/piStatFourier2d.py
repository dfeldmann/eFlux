...
#           based on 2d Fourier space cut-off filter, subtract mean velocity profile,
#           filter, compute flux, compute statistics (mean, rms, skewness, flatness)
#           plot and print to file
# Usage:    python piStatFourier2d.py 
...
fnam = 'piStatFourier2d'+'{:08d}'.format(iFirst)+'to'+'{:08d}'.format(iLast)+'nt'+'{:04d}'.format(nt)+'.dat'
f = open(fnam, 'w')
f.write('# First to fourth order one-point statistics (mean, rms, skewness, flatness) for the\n')
f.write('# Inter-scale energy flux Pi^l based on a 2d Fourier space cut-off filter)\n')
...
if plot != 2:
 ...
 fig.tight_layout()
 fnam = 'piStatFourier2d'+'{:08d}'.format(iFirst)+'to'+'{:08d}'.format(iLast)+'nt'+'{:04d}'.format(nt)+'.pdf'
 plt.savefig(fnam)
