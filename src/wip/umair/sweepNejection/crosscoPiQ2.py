...
nt = 0
acE15  = np.zeros(nz) # initialized for the correlation coefficient
acS15  = np.zeros(nz) # initialized for the correlation coefficient
...
#        acy5  = acy5  + crosscorr(uv5,   eF) # compute auto-correaltion and sum up average      
    ...
    for l in range(nth):
        ...
        uvE15  = uvE[k, l, :]
        uvS15  = uvS[k, l, :]
        acE15 = acE15  + crosscorr(uvE15, eF) # compute auto-correaltion and sum up average      
        acS15 = acS15  + crosscorr(uvS15, eF) # compute auto-correaltion and sum up average
...
ax1.plot(dz, acE15, color=Blue, linestyle='-') # .format(lambdaTh, lambdaZ))
#ax1.legend(loc='best')
...
ax2.plot(dz, acS15, color=Blue, linestyle='-') # .format(lambdaTh, lambdaZ))
#ax2.legend(loc='best')
