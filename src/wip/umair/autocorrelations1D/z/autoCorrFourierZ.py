...
nt = 0
acUz   = np.zeros(nz) # initialized for the correlation coefficient
acUzF  = np.zeros(nz) # initialized for the correlation coefficient
acPi   = np.zeros(nz) # initialized for the correlation coefficient
acOmgZ = np.zeros(nz)
...
for iFile in iFiles:
    ...
    for l in range(nth):
        ...
        uzF    = u_zF [k, l, :]
        omg    = omgZ [k, l, :]
        acUz   = acUz   + crosscorr(uz, uz)   
        acUzF  = acUzF  + crosscorr(uzF, uzF) 
        acPi   = acPi   + crosscorr(eF, eF)   
...
fnam = 'autoCorr1dFourierZ'+'{:08d}'.format(iFirst)+'to'+'{:08d}'.format(iLast)+'nt'+'{:04d}'.format(nt)+'.dat'
print('-------------------------------------------------------------------------')
