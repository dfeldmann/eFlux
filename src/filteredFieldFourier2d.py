...
# Usage:    python filteredFieldFourier2d.py
...
print('Compute filtered flow field (Fourier) for', len(iFiles), 'snapshot(s):', iFiles[0], 'to', iFiles[-1])
...
for iFile in iFiles:
    ...
    pF    = f2.fourier2d(p,    lambdaTh, lambdaZ, r, th, z)
    print('Time elapsed:', '{:3.1f}'.format(timeit.default_timer()-t1), 'seconds')

    # store result as individual HDF5 file
    fnam = 'filteredFieldFourier2d_pipe0002_'+'{:08d}'.format(iFile)+'.h5'
