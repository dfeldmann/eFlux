...
for iFile in iFiles:
      ...
      for j in range(nth-1):
        for i in range(nr-1):
         ...
         if u_z[i,j,k]<0 and u_r[i,j,k]>0: #ejections
          ...
          uvE[i,j,k] = 0.0
         if (u_z[i,j,k]>0 and u_r[i,j,k]<0) or (u_z[i,j,k]<0 and u_r[i,j,k]>0):
          uv [i,j,k] = u_z[i,j,k]*u_r[i,j,k]
         else:
          uv [i,j,k] = 0.0
    #-------------------------------------------------------------------------------
    ...
    piP = np.zeros((u_z.shape))
    piN = np.zeros((u_z.shape))
    for k in range(nz-1):
      for j in range(nth-1):
        for i in range(nr-1):
         if pi[i,j,k]>0:
    ...
    eP     = piP[k, :, :]
    eN     = piN[k, :, :]
    eF     = pi [k, :, :]  # extract 2d filtered eflux 
    uvE15  = uvE[k, :, :]
    uvS15  = uvS[k, :, :]
    uv15   = uv [k, :, :]
    print('-------------------------------------------------------------------------')
...
# fig2.tight_layout()
# fnam2 = 'efluxPQ2Fourier'+'{:08d}'.format(iLast)+'.pdf'
# plt.savefig(fnam2)
# print('Written file', fnam2)
# fig2.clf()

# fig3.tight_layout()
# fnam3 = 'efluxNQ2Fourier'+'{:08d}'.format(iLast)+'.pdf'
# plt.savefig(fnam3)
# print('Written file', fnam3)
# fig3.clf()

# fig4.tight_layout()
# fnam4 = 'efluxPQ4Fourier'+'{:08d}'.format(iLast)+'.pdf'
# plt.savefig(fnam4)
# print('Written file', fnam4)
# fig4.clf()

# fig5.tight_layout()
# fnam5 = 'efluxNQ4Fourier'+'{:08d}'.format(iLast)+'.pdf'
# plt.savefig(fnam5)
# print('Written file', fnam5)
# fig5.clf()
print('=============================================================================================')
